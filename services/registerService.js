...
exports.registerUser = async (name, email, password) => {
  ...
  const verificationUrl = `${process.env.API_URL || "http://localhost:4040"}/api/v1/auth/verify/${verificationToken}`;
...
exports.getGoogleAuthUrl = () => {
  ...
  logger.debug("Google OAuth URL generated for registration", { redirect_uri: options.redirect_uri });
...
exports.getGithubAuthUrl = () => {
  ...
  const queryString = new URLSearchParams(options).toString();
  logger.debug("GitHub OAuth URL generated for registration", { redirect_uri: options.redirect_uri });
  return `${rootUrl}?${queryString}`;
};
